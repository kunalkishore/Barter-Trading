...
<?php
	$other_error = 0;
	$gadget_name = $model = $description = $year_bought = $issues = $brand = $temp_name =  "";
	$gadget_nameErr = $modelErr = $descriptionErr = $year_boughtErr = $issuesErr = $brandErr = "";
	$target_path = "NA";
	session_start();
	//$_SESSION['key'] = 1;
	...
	if(isset($_SESSION['key']) && isset($_SESSION['validation']))
	{
		...
		else
		{
			if(isset($_POST['add_button']))
			{
				...
				{
					if (empty($_POST["gadget_name"])) {
		     			$gadget_nameErr = "Gadget name is required";
		     			$other_error = 1;
					} else {
		     			$gadget_name = test_input($_POST["gadget_name"]);
		     		}
		     		...
		     		if (empty($_POST["year_bought"])) {
		     			...
		     			if (!preg_match("/^[0-9]*$/",$year_bought) || strlen($year_bought)!=4) {
		       				$year_boughtErr = "Only legitimate years are allowed of length 4!";
		       				$other_error = 1; 
		     			}
		     		}
					...
					if($other_error == 0)
					{
						...
						{
							$sql = "INSERT INTO gadgets (gadget_name,model,year_bought,image,brand,issues,description,date_upload,user_id,first_name,rating) VALUES ('".$gadget_name."','".$model."','".$year_bought."','".$target_path."','".$brand."','".$issues."','".$description."',now(),'".$_SESSION['key']."','".$first_name."','".$rating."')";
							//echo $sql."\n";
							if ($con->query($sql) == TRUE) {
								...
								if ($con->query($sql_update) == TRUE) {
	    							echo  "<script type='text/javascript'>alert('New Gadget has been added to the record.')</script>";
...
<body>
	...
	<div id = "add">
		<form name='add_form' action='add_gadget.php' method="post" enctype='multipart/form-data'>
			<label class="username">Gadget Name: </label><input class="textbox" type="text" value="<?php echo $gadget_name;?>" name="gadget_name">
			<font size="1"><span class="error">* <?php echo $gadget_nameErr;?></span></font><br>
			<br>
			...
			<br><br><br>
			<input type="submit" class="css_button" value="Add Gadget" name="add_button">
		</form>		
	</div>
	<div id="footer1">
	</div><br>
