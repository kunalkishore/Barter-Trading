...
<?php
	...
	if(isset($_SESSION['key']) && isset($_SESSION['validation']))
	{
		...
		else
		{
			...
			$b = 0;
			$a = 0;
			if(isset($_GET['gadget_name']))
			{
				$b=1;
				$gadget_name = $_GET['gadget_name'];
				...
				else
				{
					...
					if($gadget_name!="")
					{
						$count = 0;
						$err = "";
						$sql = "SELECT * FROM gadgets WHERE (gadget_name LIKE '%$gadget_name%' and status = 0 and user_id!='".$id."')";	//0 status inplies gadget is available
			          	$result = $con->query($sql);
						if ($result->num_rows > 0) {
	    				// output data of each row
		    				while($row = $result->fetch_assoc()) {
		    					 $new_array[$count]['id'] = $row['id'];
		        				 $new_array[$count]['gadget_name'] = $row['gadget_name'];
			...
			else
			{
				...
				else
				{
					...
					if($model!="")
					{
						$count = 0;
						$err = "";
						$sql = "SELECT * FROM gadgets WHERE (model LIKE '%$model%' and status = 0 and user_id!='".$id."')";	//0 status inplies gadget is available
			          	$result = $con->query($sql);
						if ($result->num_rows > 0) {
	    				// output data of each row
		    				while($row = $result->fetch_assoc()) {
...
<body>
	...
	<div id = "home">
		<form method="post" action="search_gadgets.php"  id="searchpage"> 
			...
			{	
				...
				while($c < $count)
					{	
						...
						if($b==1)
							echo "<td><a href = 'gadgets_details.php?id=".$new_array[$c]['id']."&b_n=".$gadget_name."' style='color:white;''>".$new_array[$c]['gadget_name']."</td>";
						else 
							echo "<td><a href = 'gadgets_details.php?id=".$new_array[$c]['id']."&a=".$model."' style='color:white;''>".$new_array[$c]['gadget_name']."</td>";
