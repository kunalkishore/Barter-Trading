...
<?php
	...
	if(isset($_SESSION['key']) && isset($_SESSION['validation']))
	{
		...
		else
		{
			...
			{
				//find all the gadgets
				if($id_gadget!=0)
				{
					$sql = "SELECT * FROM gadgets WHERE id = '".$id_gadget."'";	//0 status inplies gadget is available
			...
			if(isset($_POST['order']))
			{
				...
				if($a<2 || $a<=(1/2 * $_SESSION['sent_successfully']))
				{
					...
					{	
						$points = $_SESSION['points'] - 4;
						$_SESSION['points'] = $points ;
						$sql = "UPDATE gadgets SET status = '1',order_by='".$id_user."', dispatched='0', delivered='0' where id = '".$id_gadget."' ";
					...
					else
					{
						...
						echo 'alert("You don\'t have sufficient points to order the gadget!")';
						echo '</script>';
						die("<script>location.href = 'login_home.php'</script>");
					}
				}
				else
				{
					...
					echo  "<script type='text/javascript'>alert('The ratio of gadgets sent and received is not maintained!. You need to send out more gadgets!')</script>";
					die("<script>location.href = 'login_home.php'</script>");
				}
			}
		}
...
<body>
	...
	<div id = "home">
		<?php 
			if(isset($_GET['b_n']))
				echo "<form method=\"post\" action='result_gadgets.php?gadget_name=".$b_n."'  id=\"detailpage\">" ; 
			else
				echo "<form method=\"post\" action='result_gadgets.php?model=".$a."'  id=\"detailpage\">" ;?>
	      	<input type="submit" name="detail_page" value="Go back" class="css_button"> 
	    </form> 
	    <div style="float:left;width:70%;">
		<h2>Welcome</h2>
		gadget Name       : <?php echo $gadget_name; ?><br>
		...
		<?php 
			if(isset($_GET['b_n']))
				echo "<form method=\"post\" action=\"gadgets_details.php?id=".$id_gadget."&b_n=".$b_n."\"  id=\"gadgetsdetails\"> ";
			else
				echo "<form method=\"post\" action=\"gadgets_details.php?id=".$id_gadget."&a=".$a."\"  id=\"gadgetsdetails\"> ";
			?>
